...
document.addEventListener('submit', async (e) => {
  if (e.target && e.target.id === 'eventForm') {
    ...
    const res = await fetch('http://localhost:3000/api/users/add-event', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function supprimerEvenement(index) {
  const username = localStorage.getItem('username');
  const res = await fetch('http://localhost:3000/api/users/delete-event', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (username && username !== 'admin') {
    const res = await fetch('http://localhost:3000/api/users/get-events', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
